...
class ExternalStubsScenariosSpec extends BaseSpec {
  ...
  Feature("Validate UKTR Responses with External Stubs") {
    ...
    Scenario(
      "Verify the response for the External Stub requests [UserType=Organisation, PLRID=XEPLR5555555555, StatusCode=201, RequestUrl=External stub, Endpoint=UKTaxReturn]"
    ) {
