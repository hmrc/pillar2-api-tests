...
import uk.gov.hmrc.api.pages.CommonPage.validateJsonSchema
import uk.gov.hmrc.api.specdef.AuthSteps.generateBearerToken
import uk.gov.hmrc.api.specdef.CommonSteps.{assertErrorResponse, assertStatusCode}
...
class UKTRScenariosSpec extends BaseSpec {
  ...
  Feature("Validate UKTR Json schemas and Responses") {
    ...
    Scenario(
      "Verify Submit UKTR responses and validate schema for all user types [UserType=Organisation, StatusCode=201, PLRID=XMPLR0000000012, JsonSchema=UKTR_201, RequestUrl=Submission Api, Endpoint=uk-tax-return]"
    ) {
...
//      validateJsonSchema("UKTR", "Response", "UKTR_201")
    }

    Scenario(
      "Verify Submit UKTR responses and validate schema for all user types [UserType=Organisation, StatusCode=500, PLRID=XEPLR0123456500, JsonSchema=Error_Response, RequestUrl=Submission Api, Endpoint=uk-tax-return]"
    ) {
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify Submit UKTR responses and validate schema for all user types [UserType=Organisation, StatusCode=400, PLRID=XEPLR0000000400, JsonSchema=Error_Response, RequestUrl=Submission Api, Endpoint=uk-tax-return]"
    ) {
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify Submit UKTR responses and validate schema for all user types [UserType=Organisation, StatusCode=401, PLRID=XEPLR0000000400, JsonSchema=Error_Response, RequestUrl=Submission Api, Endpoint=uk-tax-return]"
    ) {
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify Submit UKTR responses and validate schema for all user types [UserType=Agent, StatusCode=201, PLRID=XEPLR5555551126, JsonSchema=UKTR_201, RequestUrl=Submission Api, Endpoint=uk-tax-return]"
    ) {
...
//      validateJsonSchema("UKTR", "Response", "UKTR_201")
    ...
    Scenario(
      "Verify Submit UKTR responses and validate schema for all user types [UserType=Agent, StatusCode=500, PLRID=XEPLR0123456400, JsonSchema=Error_Response, RequestUrl=Submission Api, Endpoint=uk-tax-return]"
    ) {
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify Submit UKTR responses and validate schema for all user types [UserType=Agent, StatusCode=400, PLRID=XEPLR0000000400, JsonSchema=Error_Response, RequestUrl=Submission Api, Endpoint=uk-tax-return]"
    ) {
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify Submit UKTR responses and validate schema for all user types [UserType=Agent, StatusCode=401, PLRID=XEPLR0000000400, JsonSchema=Error_Response, RequestUrl=Submission Api, Endpoint=uk-tax-return]"
    ) {
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify Submit UKTR responses and validate schema for all user types [UserType=Individual, StatusCode=403, PLRID=XEPLR0000000400, JsonSchema=Error_Response, RequestUrl=Submission Api, Endpoint=uk-tax-return]"
    ) {
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify Amend UKTR responses and validate schema [UserType=Organisation, StatusCode=200, PLRID=XMPLR0000000012, JsonSchema=AmendUKTR_200, RequestUrl=Amend UKTR, Endpoint=uk-tax-return]"
    ) {
...
//      validateJsonSchema("UKTR", "Response", "AmendUKTR_200")
    }

    Scenario(
      "Verify Amend UKTR responses and validate schema [UserType=Organisation, StatusCode=500, PLRID=XEPLR0500000000, JsonSchema=Error_Response, RequestUrl=Amend UKTR, Endpoint=uk-tax-return]"
    ) {
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify Amend UKTR responses and validate schema [UserType=Organisation, StatusCode=400, PLRID=XEPLR0000000400, JsonSchema=Error_Response, RequestUrl=Amend UKTR, Endpoint=uk-tax-return]"
    ) {
      Given("I have generated a bearer token for an Organisation and XEPLR0000000400 and 400")
      generateBearerToken("Organisation", "XEPLR0000000400", "400")
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify Amend UKTR responses and validate schema [UserType=Organisation, StatusCode=401, PLRID=XEPLR0000000400, JsonSchema=Error_Response, RequestUrl=Amend UKTR, Endpoint=uk-tax-return]"
    ) {
      Given("I have generated a bearer token for an Organisation and XEPLR0000000400 and 401")
      generateBearerToken("Organisation", "XEPLR0000000400", "401")
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify Amend UKTR responses and validate schema [UserType=Individual, StatusCode=403, PLRID=XEPLR0000000400, JsonSchema=Error_Response, RequestUrl=Amend UKTR, Endpoint=uk-tax-return]"
    ) {
      Given("I have generated a bearer token for an Individual and XEPLR0000000400 and 403")
      generateBearerToken("Individual", "XEPLR0000000400", "403")
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify Submit UKTR Nil Return responses and validate schema [UserType=Organisation, StatusCode=201, PLRID=XMPLR0000000012, JsonSchema=NilReturn_201, RequestUrl=Submission Nil Return Api, Endpoint=uk-tax-return]"
    ) {
      Given("I have generated a bearer token for an Organisation and XMPLR0000000012 and 201")
      generateBearerToken("Organisation", "XMPLR0000000012", "201")
...
//      validateJsonSchema("UKTR", "Response", "Submission Nil Return Api")
    ...
    Scenario(
      "Verify Submit UKTR Nil Return responses and validate schema [UserType=Organisation, StatusCode=500, PLRID=XEPLR0123456500, JsonSchema=Error_Response, RequestUrl=Submission Nil Return Api, Endpoint=uk-tax-return]"
    ) {
      Given("I have generated a bearer token for an Organisation and XEPLR0123456500 and 500")
      generateBearerToken("Organisation", "XEPLR0123456500", "500")
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify Submit UKTR Nil Return responses and validate schema [UserType=Organisation, StatusCode=400, PLRID=XEPLR0000000400, JsonSchema=Error_Response, RequestUrl=Submission Nil Return Api, Endpoint=uk-tax-return]"
    ) {
      Given("I have generated a bearer token for an Organisation and XEPLR0000000400 and 400")
      generateBearerToken("Organisation", "XEPLR0000000400", "400")
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify Submit UKTR Nil Return responses and validate schema [UserType=Organisation, StatusCode=401, PLRID=XEPLR0000000400, JsonSchema=Error_Response, RequestUrl=Submission Nil Return Api, Endpoint=uk-tax-return]"
    ) {
      Given("I have generated a bearer token for an Organisation and XEPLR0000000400 and 401")
      generateBearerToken("Organisation", "XEPLR0000000400", "401")
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify Submit UKTR Nil Return responses and validate schema [UserType=Individual, StatusCode=403, PLRID=XEPLR0000000400, JsonSchema=Error_Response, RequestUrl=Submission Nil Return Api, Endpoint=uk-tax-return]"
    ) {
      Given("I have generated a bearer token for an Individual and XEPLR0000000400 and 403")
      generateBearerToken("Individual", "XEPLR0000000400", "403")
...
//      validateJsonSchema("UKTR", "Response", "Error_Response")
    ...
    Scenario(
      "Verify the error code & message for the invalid UKTR requests [UserType=Organisation, Enrolment=with enrolment, StatusCode=400, ErrorCode=INVALID_JSON, ErrorMessage=Invalid JSON payload, PLRID=XMPLR0012345674]"
    ) {
    ...
    Scenario(
      "Verify the error code & message for the invalid UKTR requests [UserType=Organisation, Enrolment=with enrolment, StatusCode=400, ErrorCode=EMPTY_REQUEST_BODY, ErrorMessage=Empty request body, PLRID=XMPLR0012345674]"
    ) {
      Given("I have generated a bearer token for an Organisation and with enrolment and 400")
      generateBearerToken("Organisation", "with enrolment", "400")
